...
module.exports = {
    ...
    .addSubcommand(subcommand => 
        ...
        .addIntegerOption(option => 
            ...
            .setDescription('Enter the numbers of winners (default is 1).')
            .setRequired(false)
        )
    ...
    .addSubcommand(subcommand => 
        ...
        .setDescription('Select a new giveaway winner.')
        .addStringOption(option => 
            option.setName('message-id')
            .setDescription('Specify giveaway message-id.')
            .setRequired(true)
    ...
    runSlash: async (interaction) => {
        ...
        if (sub === 'end') {
            client.giveawaysManager.end(messageid).then(() => {
                successEmbed.setDescription('The giveaway has been stopped.').setColor('Blue');
                return interaction.reply({embeds: [successEmbed], ephemeral: true});
            }).catch((err) => {
                console.log(err)
                errorEmbed.setDescription(`Error \n\`${err}\``);
